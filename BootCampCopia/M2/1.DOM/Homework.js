...
//document.querySelector('#createdBy').innerHTML += ' Jimy';
...
function buildToDo(todo, index) { // todo: ToDo{description: ' Welcome ', complete:false} , índex:núm
 // Tu código acá:
    // Paso 1
    ...
    // Paso 4
    toDoText.innerHTML = todo.description // <span> Welcome </span>
    // Paso 5
    toDoText.id = index // <span id=0 > Welcome </span>
    // Paso 6
    if (todo.complete) toDoText.className.add = 'completeText' // directamente todo.complete porque quedo como un booleano cuando hicimos el prototipo.
    // Paso 7
    toDoShell.appendChild(toDoText) 
    // <div class = 'toDoShell'>
    // <span id=0 > Welcome </span>
    // </div>
    toDoText.addEventListener('click', completeToDo)
    // Paso 8
    return toDoShell //retorno todo lo que le sigue al paso 7
  }
...
function displayToDos() {
  ...
  let result = buildToDos(toDoItems)
   // Paso 4
  for(let i = 0; i < result.length; i++) { //buildToDos(toDoItems).forEach(element => toDoContainer.appendChid(element))
...
function addToDo() {
  ...
  if(input.value !== '') { // si no es un string vacío no hacer nada.
...
function completeToDo(event) {
 ...
 // a) Creamos un elemento input del tipo checkbox y lo asignamos a la variable completeCheckbox:
const completeCheckbox = document.createElement('input');
completeCheckbox.type = 'checkbox';

//b) Asignamos el valor del índice al id del completeCheckbox y quitamos el id del toDoText:
completeCheckbox.id = index;
toDoText.removeAttribute('id');

//c) Agregamos el Event Listener de completeToDo al completeCheckbox y removemos el Event Listener de completeToDo del toDoText:
completeCheckbox.addEventListener('click', completeToDo);
toDoText.removeEventListener('click', completeToDo);

//d) Agregamos la clase completeCheckbox al completeCheckbox:
completeCheckbox.classList.add('completeCheckbox');

//e) Si el objeto ToDo está completo (isComplete es true), seteamos el atributo checked del completeCheckbox en true:
if (todo.isComplete) {
  completeCheckbox.checked = true;
}

//f) Agregamos el completeCheckbox al elemento toDoShell:
toDoShell.appendChild(completeCheckbox);

//Finalmente, después del bloque if de la función buildToDo, llamamos a la función displayToDos para actualizar la vista:
if (todo.isComplete) {
  completeCheckbox.checked = true;
}
toDoShell.appendChild(completeCheckbox);
